...
{
    public class Stair3DCreator
    {

        #region 构建 3D双跑楼梯
        /// <summary>
        /// 构建 3D双跑楼梯（依次返回休息平台、梯段、立柱、扶手几何，其相对位置均以调整正确）
        /// </summary>
        /// <param name="restPlatform"></param>
        /// <param name="stairFlight"></param>
        /// <param name="stairRailing"></param>
        /// <returns></returns>
        public static List<IGeometry> Create_DRunningStair(StairParameters.DStairParameter dsp)
        {
            List<IGeometry> geometries = new List<IGeometry>();

            //使休息平台宽为梯段宽之和+梯段缝隙宽
            dsp.Platform.Width = 2 * dsp.Fligth.FlightWidth + dsp.Fligth.StairwellWidth;

            IVector3D pRotateVector = ArcObjectsUtilities.GeometryUtility.Axis_Vector(3);
            double rotateAngleInRadian = -Math.PI / 2;

            //1.    构造休息平台
            ITransform3D stairLanding_transform3D = Construct_StairLanding(dsp.Platform) as ITransform3D;

            //2.    构造梯段
            ITransform3D stairFlight_transform3D = Construct_StairSection(dsp.Fligth) as ITransform3D;

            //3.    构造栏杆(栏杆柱+扶手)
            ITransform3D verticalColumns3D_transform3D = Construct_VerticalColumns(ref  dsp.Railing.Baluster) as ITransform3D;
            ITransform3D handrails3D_transform3D = Construct_Handrail(ref dsp.Railing.Handrail) as ITransform3D;



            //4.    相对位置调整
            verticalColumns3D_transform3D.Move3D(
               dsp.Platform.Depth - dsp.Railing.Baluster.GapWidth / 2,
                 dsp.Platform.Width / 2,
                 0
                );
            handrails3D_transform3D.Move3D(
                 dsp.Platform.Depth - dsp.Railing.Baluster.GapWidth / 2,
                 dsp.Platform.Width / 2,
               dsp.Railing.Baluster.Height);

            stairFlight_transform3D.Move3D(
                 dsp.Platform.Depth,
                0,
                -dsp.Fligth.StepHeight);


            /*5. 由于该算法构造的双跑楼梯原型位置左上角位于坐标原点，左侧梯段(面向上楼方向)贴合X轴，
             *   因此在移动到WCS中之前，应先将其围绕Z轴旋转90度，使得楼梯呈南北走向，上楼方向朝北
            */

            //旋转休息平台
            stairLanding_transform3D.RotateVector3D(pRotateVector, rotateAngleInRadian);

            //旋转梯段
            stairFlight_transform3D.RotateVector3D(pRotateVector, rotateAngleInRadian);

            //旋转立柱和扶手
            verticalColumns3D_transform3D.RotateVector3D(pRotateVector, rotateAngleInRadian);
            handrails3D_transform3D.RotateVector3D(pRotateVector, rotateAngleInRadian);


            //收集双跑楼梯 4大 构件
            geometries.Add(stairLanding_transform3D as IGeometry);
            geometries.Add(stairFlight_transform3D as IGeometry);
            geometries.Add(verticalColumns3D_transform3D as IGeometry);
            geometries.Add(handrails3D_transform3D as IGeometry);
        ...
        static IGeometry Construct_VerticalColumns(ref StairParameters.Baluster baluster)
        {
            IGeometry columns;
